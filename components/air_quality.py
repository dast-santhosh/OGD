...
def create_heat_map(stakeholder):
    ...
    with col1:
        st.selectbox("Time Period:", ["Current", "Daily Average", "Weekly Trend", "Monthly Comparison"], key="time_period")
    with col2:
        st.selectbox("Temperature Layer:", ["Surface Temperature", "Air Temperature", "Heat Index", "Temperature Anomaly"], key="temp_layer")
    with col3:
        st.selectbox("Overlay Data:", ["None", "NDVI (Vegetation)", "Population Density", "Building Density"], key="overlay_data")
    ...
    # Add heat map layer (fix randomness)
    np.random.seed(42)
    heat_data = []
